---
name: humanizer-zh
description: 去除文本中的 AI 生成痕迹。适用于编辑或审阅文本，使其听起来更自然、更像人类书写。基于维基百科的"AI 写作特征"综合指南，并补充了中文特有的高危模式。检测并修复以下模式：夸大的象征意义、宣传性语言、以 -ing 结尾的肤浅分析、模糊的归因、破折号过度使用、三段式法则、AI 词汇、过多的连接性短语；以及四类中文高危信号——"不是……而是……"式否定排比、抽象概念拟人化加冒号、"真正……的是……"倒装强调、程度副词加单双字词的空洞评价。
...
### 9. 否定式排比 / "不是……而是……"（重点，几乎已不可用）

**问题：** AI 过于频繁地使用"先否定再肯定"的结构，滥用到这个句式本身现在几乎完全不能用了。包括：
...
**为什么它是强信号：** 人类想表达一件事时，通常直接说那件事。先立一个靶子再打掉，是模型在制造"有洞察感"的廉价手法。一篇文章里出现一次可能是巧合，出现三次以上基本可以判定为 AI 生成。

**核心原则：如果你要否定的那个东西根本没人主张过，就不要否定它。**
...
> 结论是，我们要的不是「skill 越多越好」，而是在正确的任务里找到正确的 skill。

**改写后：**

> 结论是，我们不能追求「skill 越多越好」，而要在正确的任务里找到正确的 skill，再 harness 真正把它执行出来。

**注意上面这组的区别：** 改写后仍然有转折，但"不能追求 X，而要 Y"是在否定一个真实存在的错误做法（确实有人在堆 skill），这是正常的人类议论。而"不是 X，而是 Y"里的 X 往往是作者自己临时造出来、只为了被推翻的稻草人。**判断标准是 X 是否真实存在，不是句式本身。**

**其他改法：**

- 直接说 B，把 A 删掉
- 把 A 降格为让步：「A 当然也有，但主要是 B」
- 改成因果或递进：「因为 A，所以 B」
...
**问题：** 把一个非人的抽象概念放在主语位置，让它像人一样"要求""需要""说话""值得"，然后跟一个冒号引出列表或解释。
...
**为什么是信号：** 人类写作时，说话的是"我"，不是"贡献边界"。这种句式把作者从句子里删掉了，让抽象名词代替人发言，制造出一种煞有介事的仪式感。

**改写前：**

> 贡献边界得说清楚：这个项目的核心算法由算法组完成，我负责工程化。

**改写后：**

> 我对这个项目也有贡献，主要在工程化这块。核心算法是算法组做的。

**改写前：**

> 有几个问题值得回答：为什么现在做？为什么是我们？

**改写后：**

> 我想回答两个问题。为什么现在做，以及为什么是我们。

**改法：** 把人放回主语位置（我/我们/团队/作者），或者干脆把冒号前的整句删掉，直接进入内容。

---

### 9c. "真正……的，都是……"（倒装强调）

**问题：** 用"真正 X 的，是 Y""真正重要的，从来都是 Z"这类倒装结构来强行制造顿悟感。同族的还有"归根结底""说到底""本质上""从来都是"。

**为什么是信号：** 人类陈述事实时直接陈述。这个句式的作用是给一句平常话套上"我要揭示一个深刻真相了"的外壳，而内容往往并不深刻。
...
> 其实是算法同事把研究、工程和实验做出来的。

**改写前：**

> 真正决定一个模型好坏的，从来都不是参数量。

**改写后：**

> 参数量不太能决定一个模型的好坏。

**改法：** 把倒装拆回正常语序，主语放前面，去掉"真正""从来""归根结底"这类加强词。

---

### 9d. 程度副词 + 单双字名词/形容词

**问题：** "很硬""很实际""极冷""特别真""相当扎实""非常干净"这类搭配。程度副词后面接一个未经展开的单字或双字词，用来给事物下一个抽象评价。

**为什么是信号：** 中文里"很 + 单字形容词"在口语中是自然的（很好、很难），但在书面评价里，"很硬""很实际"这种搭配相当生硬——人一般会说清楚硬在哪、实际在哪。这是模型在用一个程度词代替具体描述。

**改写前：**

> 这篇论文很实际。/ 这条证据很硬。/ 这个数据很扎实。

**改写后：**

> 这篇论文研究的问题和实际很贴近。/ 实验验证了这条证据。/ 这个数据的样本量够大，也做了对照。

**改法：** 删掉程度副词，把那个抽象评价展开成一句具体的话。说清楚"硬"体现在什么地方，比说"很硬"有用得多。

**⚠️ 展开时不要落进 9 号的坑。** 把抽象词展开成具体描述时，很容易顺手写成"是 A，不是 B"的对照句，那等于修掉一个 AI 信号又装回另一个。

- ❌ 这条证据是实验测出来的，不是推理出来的。
- ✅ 实验验证了这条证据。

直接说做了什么就行，不用把没做的那件事也搬出来垫背。
...
改完之后，用文本搜索逐条过一遍。这四项是中文语境下最容易暴露的，且模型改写时极易复发：

1. **搜"不是"、"不仅"、"而是"、"与其"** —— 每一处都要问：被我否定的那个东西，真的有人主张过吗？没有就删掉否定句，直接说结论。
2. **搜冒号"："** —— 冒号前是不是一个抽象概念在"要求"或"值得"什么？是的话把"我/我们"放回主语，或者删掉整个引子。
3. **搜"真正"、"从来"、"归根结底"、"说到底"、"本质上"** —— 基本都可以直接删掉，句子会更清楚。
4. **搜"很"、"极"、"非常"、"特别"、"相当"** —— 后面跟的是不是一个没展开的单双字词（很硬/很实际/极冷）？是的话展开成具体描述。

另外两项通用的：

5. **数一下加粗** —— 一屏之内超过两三处就是过量。
6. **数一下引用块** —— 如果每一节都以一句金句式引用收尾，那是机器节奏，留一到两处即可。

### 改完再搜一遍第 1 项

修 2、3、4 项的时候，手会不自觉地把内容重写成对照句，于是第 1 项的数量反而变多了。典型的复发形态：

- 修"很硬"→ 写成"是实验测出来的，不是推理出来的"（应该写"实验验证了这条证据"）
- 修"真正重要的是 X"→ 写成"重要的不是 Y，是 X"（应该直接写"X 很重要"）

所以第 1 项要搜两遍：改之前一遍，全部改完之后再一遍。
