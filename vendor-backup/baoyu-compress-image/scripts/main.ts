...
async function stripExifWithExiftool(file: string): Promise<void> {
  if (!(await commandExists("exiftool"))) return;
  await runCmd("exiftool", ["-all=", "-overwrite_original", "-q", file]);
}

async function compressWithSips(input: string, output: string, format: Format, quality: number, stripMetadata: boolean): Promise<void> {
  ...
  if (stripMetadata) await stripExifWithExiftool(output);
}

async function compressWithCwebp(input: string, output: string, quality: number, stripMetadata: boolean): Promise<void> {
  const args = ["-q", String(quality)];
  if (stripMetadata) args.push("-metadata", "none");
  args.push(input, "-o", output);
  const { code, stderr } = await runCmd("cwebp", args);
  if (code !== 0) throw new Error(`cwebp failed: ${stderr}`);
}

async function compressWithImagemagick(input: string, output: string, quality: number, stripMetadata: boolean): Promise<void> {
  const args = [input, "-quality", String(quality)];
  if (stripMetadata) args.push("-strip");
  args.push(output);
...
async function compressWithSharp(
  ...
  if (clean) {
    ...
    if (w > 0 && h > 0) {
      pipeline = pipeline
        .resize(Math.max(1, Math.round(w * 0.97)), Math.max(1, Math.round(h * 0.97)), { kernel: "lanczos3" })
        .resize(w, h, { kernel: "lanczos3" })
        .modulate({ brightness: 1.0, saturation: 1.0, hue: 1 })
        .blur(0.3);
    }
  }
...
async function applyCleanPassImagemagick(file: string, quality: number, stripMetadata: boolean): Promise<void> {
  const tmp = file + ".clean.tmp" + extname(file);
  const args = [
    file,
    "-filter", "Lanczos",
    "-resize", "97%",
    "-resize", "103.092783%",
    "-modulate", "100,100,101",
    "-blur", "0x0.3",
  ...
  args.push(tmp);
  const { code, stderr } = await runCmd("convert", args);
  if (code !== 0) throw new Error(`convert (clean pass) failed: ${stderr}`);
  renameSync(tmp, file);
}

async function applyCleanPass(file: string, format: Format, quality: number, stripMetadata: boolean): Promise<void> {
  try {
    const sharp = (await import("sharp")).default;
    ...
    if (await commandExists("convert")) {
      await applyCleanPassImagemagick(file, quality, stripMetadata);
    } else {
      throw new Error("--clean requires sharp or imagemagick (convert)");
    }
  }
}
...
async function compress(
  ...
  switch (compressor) {
    case "sips":
      await compressWithSips(input, output, format, quality, stripMetadata);
      if (clean) await applyCleanPass(output, format, quality, stripMetadata);
      break;
    case "cwebp":
      if (format !== "webp") {
        await compressWithSharp(input, output, format, quality, stripMetadata, clean);
      } else {
        await compressWithCwebp(input, output, quality, stripMetadata);
        if (clean) await applyCleanPass(output, format, quality, stripMetadata);
      }
      break;
    case "imagemagick":
      await compressWithImagemagick(input, output, quality, stripMetadata);
      if (clean) await applyCleanPass(output, format, quality, stripMetadata);
      break;
    case "sharp":
      await compressWithSharp(input, output, format, quality, stripMetadata, clean);
...
async function processFile(
  ...
  await compress(compressor, absInput, tempOutput, opts.format, opts.quality, opts.stripMetadata, opts.clean);
...
Options:
  ...
  -r, --recursive       Process directories recursively
      --keep-metadata   Preserve EXIF/metadata (default: stripped)
      --clean           Extra pass to weaken invisible AI watermarks (e.g., SynthID).
                        Lossy: resample + slight blur + re-encode. Best-effort, not guaranteed.
      --json            JSON output
  -h, --help            Show help`);
}
