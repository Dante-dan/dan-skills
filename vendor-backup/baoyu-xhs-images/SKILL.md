...
## 🚫 Platform Compliance (Xiaohongshu)

**No external URLs in image content.** Xiaohongshu's content moderation flags images containing personal blog domains, full URLs, or QR codes pointing offsite as malicious external linking (恶意外链), which suppresses reach or removes the post.

Apply to ALL prompts (especially the ending/CTA card):

| ❌ Don't put in image | ✅ Use instead |
|----------------------|---------------|
| `dhpie.com/posts/xxx` | "完整版见个人主页" / "全文见主页 👉" |
| `https://example.com` | Plain text CTA without URL |
| QR code to external site | Hand-drawn arrow + "主页" reference |
| `@user` handles to other platforms | Generic "关注我" |

Where the URL belongs: in the post **caption** (text body submitted alongside images) or in the user's Xiaohongshu profile bio — NOT baked into the image itself.

When generating the ending/CTA card, the prompt MUST omit URLs and use phrases like "完整版见主页", "全文戳主页", or "更多内容主页查看". Reviewers/Claude must reject any prompt containing a domain/URL string in the image text.
...
- Provider-claimed file format (e.g. `.png`) often disagrees with actual format (e.g. JPEG with PNG extension) — verify and rename
...
1. Detect actual format with `PIL.Image.open(f).format` (not the extension)
2. Re-save via PIL with no metadata (this is the EXIF-strip step):
   ```python
   ...
   ```
3. If actual format ≠ claimed extension, rename to the correct one and remove the original
4. **Verify**: re-open the saved file and assert `img.getexif()` is empty AND `img.info` has no metadata keys (other than benign jfif/dpi)
5. Only after verification passes may you report the image as done
