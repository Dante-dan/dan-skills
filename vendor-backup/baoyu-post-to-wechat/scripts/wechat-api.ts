...
async function publishToDraft(
  ...
  if (options.articleType === "newspic") {
    ...
    article = {
      ...
      need_open_comment: options.needOpenComment ?? 1,
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--type" && argv[i + 1]) {
      ...
      args.cover = argv[++i];
    } else if (arg === "--content-source-url" && argv[i + 1]) {
...
async function main(): Promise<void> {
  ...
  const contentSourceUrl = args.contentSourceUrl || frontmatter.link || frontmatter.content_source_url || "";
